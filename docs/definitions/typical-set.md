...
The typical set contains Young diagrams whose normalized shape $\lambda/n$ is close to the eigenvalue distribution $p$. By [[results/remaining-lemmas|Sanov's Theorem (Lemma 12)]], the probability of being outside $T_{p,n}$ is exponentially small: $\Pr[\lambda \notin T_{p,n}] \leq (n+1)^{r(r+1)/2} e^{-2n\xi_n^2/n}$.
