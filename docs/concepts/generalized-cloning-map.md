...
See [[results/propositions/reverse-cloner|Reverse Cloner (Prop 2)]].

### 4. Output Commutes with Target
For any $U(d)$-covariant channel, $[C_{\mu \to \nu}(\rho_\mu), \rho_\nu] = 0$. The output always commutes with the target state, so the cloner only distorts eigenvalues, not eigenbases. See [[results/propositions/commutativity|Commutativity (Prop 3)]].
...
- [[results/propositions/reverse-cloner|Reverse Cloner (Prop 2)]] -- Petz recovery interpretation
- [[results/propositions/commutativity|Commutativity (Prop 3)]] -- structural property
